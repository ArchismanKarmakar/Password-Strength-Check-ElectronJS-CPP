...
static double CalcPass(const char *Pwd, int Quiet)
{
    ...
    if (p)
    {
        ...
        case DATE_MATCH + MULTIPLE_MATCH:
            ...
            break;
        case YEAR_MATCH + MULTIPLE_MATCH:
            // printf("  Type: Year(Rep)      ");
            break;
        case LONG_PWD_MATCH + MULTIPLE_MATCH:
            // printf("  Type: Extra-long(Rep)");
            break;
...
double passStrengthChk::passStrengthChk(string x, string yVoid)
{
    ...
    //     }
    // }
    char* pwd = (char*)x.c_str();
    double entropy = CalcPass(pwd, Quiet);
    ZxcvbnUnInit();
    return entropy;
}
...
// return (log2(lwCase + upCase + num + special) * x.length());